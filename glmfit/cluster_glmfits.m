function cluster_glmfits(fits_table,dspec,varargin)
    ...
    for i=1:numel(params.covariates_to_plot)  
        ...
        if i==1
            these_cols = 1:edim(1);
            pos = [0.115 0.1 4.2e-2*time_range(i) 0.8];     % change this 4e-2 scaling value if you want to stretch the images more or less        
        else
            these_cols = (edim(i-1)+1) : edim(i);
            pos = [pos(1)+gap+pos(3) 0.1 4.2e-2*time_range(i) 0.8]; % and here. this is a bit hacky and i should automatically pick this value to cover the desired range of the images in the plot          
        end
        img_axes(i) = axes('Position',pos,'ydir','reverse');                  
        if contains(params.covariates_to_plot{i},"click")        
        ...
        if contains(params.covariates_to_plot{i},"click")
            ...
            line(ones(1,2)*zr(1),yl,'LineWidth',1.5,'LineStyle','--','color',ones(1,3)/2);
        else
            set(gca,'xtick',covariate_ticks,P.axes_properties{:});box off;   
            line([0 0],yl,'LineWidth',1.5,'LineStyle','--','color',ones(1,3)/2);            
        end
        if params.reorder_cells
            for k=1:(ngroups-1)
                line(xl,ones(1,2)*linepos(k),'Color','k','LineWidth',1.5 );
            end            
        else          
        ...
        ax.FontSize = 14;           
        title(strsplit(params.covariate_names{i},' '),'FontSize',20);
        if i == numel(params.covariates_to_plot)
    ...
    if ~params.reorder_cells
        ...
        if params.variable_bar_size
            for i=1:numel(linepos)
                if i==1
                    real_width = 0.8*linepos(i)./ncells;
                else
                    real_width = 0.8*diff(linepos([i-1 i]))./ncells;           
                end
                if real_width>0.03
