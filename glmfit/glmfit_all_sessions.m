    ...
    for i=1:length(paths)
        ...
        if p.Results.job_array && P.on_cluster
            matlab_command = sprintf(['fit_glm_to_Cells(''%s'',''save_path'',''%s'',''save'',true,''bin_size_s'',%0.10g,',...
                '''kfold'',%g,''phi'',%0.10g,''tau_phi'',%0.10g,''choice_time_back_s'',%0.10g,'...
                ...
                '''stim_period_mode'',''%s'',''separate_clicks_by_outcome'',logical(%g),''lambda'',%0.10g,''lambda_correct_zscore'',logical(%g));'],...
                paths(i).cells_file,output_dir,params.bin_size_s,params.kfold,params.phi,...
                params.tau_phi,params.choice_time_back_s,params.distribution,params.include_mono_clicks,params.use_trial_history,...
                params.nClickBins,params.separate_clicks_by,params.confine_clicks_to_stim_period,params.stim_period_boundary{:},...
                params.stim_period_mode,params.separate_clicks_by_outcome,params.lambda,params.lambda_correct_zscore);
            ...
            sbatch_command = sprintf('sbatch -e %s -o %s -t %g -J "%s" --array=%s --mem-per-cpu=5G submit_matlab_job.slurm %s',error_file,out_file,round(p.Results.time_per_job*60),job_name,array_string(1:end-1),matlab_command);
        else
            ...
            matlab_command = sprintf(['"fit_glm_to_Cells(''%s'',''save_path'',''%s'',''save'',true,''bin_size_s'',%g,',...
                ...
                '''distribution'',''%s'',''include_mono_clicks'',logical(%g),''use_trial_history'',logical(%g),''nClickBins'',%g,',...
                '''separate_clicks_by'',''%s'',''confine_clicks_to_stim_period'',logical(%g),''stim_period_boundary'',{''%s'',''%s''},'...
                '''stim_period_mode'',''%s'',''separate_clicks_by_outcome'',logical(%g),''lambda'',%0.10g,''lambda_correct_zscore'',logical(%g));exit;"'],...
                ...
                params.separate_clicks_by_outcome,params.lambda,params.lambda_correct_zscore);       
