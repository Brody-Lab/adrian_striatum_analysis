function [cells_mat,params] = MakeDataMatrix(Cells,varargin)
    ...
    p.addParameter('resolution_s',0.02,@(x)validateattributes(x,{'numeric'},{'nonnegative'}));
    ...
    params.trial_idx = params.trial_idx(:)' & ~isnan(Cells.Trials.stateTimes.(params.ref_event))';
    
    params.units=params.units(:)';
    params.time_edges_s = params.time_window_s(1):params.resolution_s:params.time_window_s(2);    
    ...
    n_time_bins = numel(params.time_edges_s)-1;
    ntrials = numel(find(params.trial_idx));
    ...
    bin_mids = (params.time_edges_s(1:end-1) + params.time_edges_s(2:end))/2;
    params.times = bsxfun(@plus,bin_mids',ref_event_times'); % times of the matrix rows in seconds according to the bcontrol clock
    ...
    for i=find(params.units)
        ...
        trial_count=0;                
        for j = find(params.trial_idx)
            trial_count = trial_count+1;
            trial_idx  = (1:n_time_bins) + n_time_bins*(trial_count-1);
            if params.average
                hold_mat(:,cell_count,trial_count) = matlab.internal.math.histcounts(spikes_i{j},params.time_edges_s); 
            else
            % using the hidden internal matlab function instead of "histcounts" avoids repeated argument validation
                cells_mat(trial_idx,cell_count) = matlab.internal.math.histcounts(spikes_i{j},params.time_edges_s);                     
            end
        end       
    end
    if params.average
