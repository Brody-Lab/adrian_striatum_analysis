function plot_reliability(params)
    ...
    arguments
        ...
        params.max_days_implanted (1,1) = Inf
    end
    P = get_parameters;
    %% load cells table
    ...
    tagged = T.reliability>0.9 & T.DV>params.min_dv & T.days_implanted<params.max_days_implanted ; % use reliability>0.9 as the metric for now
    untagged = T.reliability<0.9 & ~isnan(T.reliability) & T.DV>params.min_dv & T.days_implanted<params.max_days_implanted;     
    ...
    legend(h,{'Not Tagged','Tagged (Putative D2+ SPN)'},'Location','northeast');  
end