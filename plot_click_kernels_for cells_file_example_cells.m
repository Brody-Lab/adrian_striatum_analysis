...
cellno = randsample([31  45 30 18 21  33 27  38],8);
...
for i=1:numel(cellno)
    ...
    mn=mean(mn);
    line([-0.05 0.2],[1 1],'color',[1 1 1]/2,'LineWidth',0.8);
        line([ 0 0], [0.75 1.25],'color',[1 1 1]/2,'LineWidth',0.8);            
    h(1)=shadedErrorBar(kPETH.timeS.left_clicks(id),psth_left(cellno(i)).left_clicks(:,id)./mn,{@mean,@std},{'color',P.gamma_color_groups(end,:)});hold on;
    try
    h(1).patch.FaceAlpha=0.8;    
    end
    mn = mean(psth_right(cellno(i)).right_clicks(:,idx));
    mn=mean(mn);    
    h(2)=shadedErrorBar(kPETH.timeS.left_clicks(id),psth_right(cellno(i)).right_clicks(:,id)./mn,{@mean,@std},{'color',P.gamma_color_groups(2,:)});
    try
    ...
    if i==numel(cellno)
        ...
        text(0.33,0.95,{'50%','change'},'FontSize',13,'HorizontalAlignment','center');
        
    end
    %title(sprintf('%g',cellno(i)));
    axis off;    
    if i==numel(cellno)-1 
        text(-0.01,0.68,'0','FontSize',13);
        text(-0.05,1,'1','FontSize',13);
        text(0.01,0.55,'Time after click','FontSize',13);
        text(-0.22,1,{'Normalized','Firing Rate'},'FontSize',13);
    else
