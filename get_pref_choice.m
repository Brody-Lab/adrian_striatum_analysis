function [choice_MI,choice_MI_pval] = get_pref_choice(Cells,cellno,varargin)
    ...
    p.addParameter('cpoke_out_window',0.1); % don't include times within 0.2 seconds of the cpoke out
    p.parse(varargin{:});
    params = p.Results;
    ...
    for c=1:numel(cellno)    
        for i=1:2
        if i==1
            these_trials = intersect(trial_idx,find(Cells.Trials.pokedR==1));
        else
            these_trials = intersect(trial_idx,find(Cells.Trials.pokedR==0));            
        end
            psth=calc_psth(Cells.spike_time_s.(eve){cellno(c)}(these_trials),0.2:0.05:1,'LGAUSS',0.1);
            nan_inds = ([0.2:0.05:1]+params.cpoke_out_window) > stim_dur(these_trials);        
            psth(nan_inds)=NaN;
            psth = nanmean(psth,2);
